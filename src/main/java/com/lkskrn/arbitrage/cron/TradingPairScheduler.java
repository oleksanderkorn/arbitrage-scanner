...
public class TradingPairScheduler {
    ...
    @Autowired
    public TradingPairScheduler(TradingAssetService tradingPairService, RestTemplate restTemplate) {
    ...
    public void loadAvailablePairs() {
        List<String> supportedAssets = getSupportedAssets();
        supportedAssets.forEach(baseAsset -> {
    ...
    public void scanPrices() {
        tradingPairService.findAll().forEach(asset -> {
            getCoinbaseTicker(asset).ifPresent(coinbaseTicker -> getBinanceTicker(asset).ifPresent(binanceTicker -> {
                ...
                BigDecimal delta = binanceBips.compareTo(coinbaseBips) > 0 ? binanceBips.subtract(binanceBips) : coinbaseBips.subtract(binanceBips);
